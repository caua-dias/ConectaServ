...
class _ReviewsPageState extends State<ReviewsPage> with SingleTickerProviderStateMixin {
  ...
  List<Map<String, dynamic>> _getRecebidas() {
    ...
    for (var av in avaliacoes) {
      ...
      final contrato = _findFirst(contratacoes, (c) => c.idContratacao == av.idContratacao);
      if (contrato == null) continue;

      // Procurar o serviço
      final servico = _findFirst(servicos, (s) => s.idServico == contrato.idServico);
      if (servico == null) continue;

      // Procurar a empresa
      final empresa = _findFirst(empresas, (e) => e.idEmpresa == servico.idEmpresa);

      result.add({
        'id': av.idAvaliacao,
        'company': empresa?.nomeFantasia ?? empresa?.cnpj ?? 'Empresa Desconhecida',
        'emoji': empresa?.emoji ?? '🏢',
        'category': _formatarNomeCategoria(servico.categoria),
        'rating': av.estrelas,
        'date': 'Recente', // Como o SQLite não tem data por defeito, usamos um placeholder
  ...
  List<Map<String, dynamic>> _getPendentes() {
    ...
    for (var contrato in contratacoes) {
      ...
      bool hasReview = avaliacoes.any((av) => av.idContratacao == contrato.idContratacao);
        ...
        builder: (ctx, setModalState) => Padding(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Text(pendente['emoji'] as String, style: const TextStyle(fontSize: 28)),
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Avaliar Empresa', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    Text(pendente['company'] as String, style: const TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                  ]),
                ]),
                const SizedBox(height: 20),
                const Text('Sua nota', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                ...
                const Text('Comentário', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                const SizedBox(height: 8),
                TextField(
                  controller: _ctrl,
                  maxLines: 4,
                  ...
                  child: ElevatedButton(
                    onPressed: _stars == 0 || _ctrl.text.trim().isEmpty
                        ...
                        : () async {
                            ...
                            await context.read<AvaliacaoNotifier>().salvar(novaAvaliacao);
                    ...
                    child: const Text('Enviar Avaliação', style: TextStyle(fontWeight: FontWeight.w600)),
                  ),
                ),
                const SizedBox(height: 8),
              ],
  ...
  Widget build(BuildContext context) {
    ...
    if (listaRecebidas.isNotEmpty) {
      int totalEstrelas = listaRecebidas.fold(0, (sum, item) => sum + (item['rating'] as int));
      media = totalEstrelas / listaRecebidas.length;
      positivas = listaRecebidas.where((item) => (item['rating'] as int) >= 4).length;
    }
    String percentagemPositiva = listaRecebidas.isEmpty ? "0%" : "${((positivas / listaRecebidas.length) * 100).toStringAsFixed(0)}%";
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Avaliações', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 17, color: Color(0xFF0F172A))),
      ...
      body: carregando 
        ...
        children: [
            ...
            children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      ...List.generate(5, (i) => Padding(
                        ...
                        child: _filtroChip(label: '${5 - i} ⭐', value: 5 - i),
                      )),
                    ],
                  ),
                ),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    _statCol(listaRecebidas.isEmpty ? '-' : media.toStringAsFixed(1), 'Nota Média', const Color(0xFFF59E0B)),
                    _dividerVertical(),
                    _statCol('${listaRecebidas.length}', 'Total', const Color(0xFF2563EB)),
                    _dividerVertical(),
                    _statCol(percentagemPositiva, 'Positivas', const Color(0xFF16A34A)),
                  ],
                ),
              ),
              Divider(height: 1, color: Colors.grey[200]),
              ...
              Expanded(
                child: listaRecebidas.isEmpty
                    ? _emptyState('Nenhuma avaliação encontrada', Icons.star_outline)
                    : ListView.separated(
                                ...
                                children: [
                                  Row(children: [
                                    Container(
                                      ...
                                      child: Center(child: Text(av['emoji'] as String, style: const TextStyle(fontSize: 22))),
                                    ),
                                    ...
                                    Expanded(
                                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                        ...
                                        Text(av['category'] as String, style: const TextStyle(color: Color(0xFF64748B), fontSize: 11)),
                                      ]),
                                    ),
                                    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                                      Row(children: List.generate(av['rating'] as int, (_) =>
                                          const Icon(Icons.star, color: Color(0xFFF59E0B), size: 13))),
                                      Text(av['date'] as String, style: const TextStyle(fontSize: 10, color: Color(0xFF94A3B8))),
                                    ]),
                                  ]),
          ...
          listaPendentes.isEmpty
              ? _emptyState('Nenhuma avaliação pendente', Icons.check_circle_outline)
              : ListView.separated(
                  padding: const EdgeInsets.all(16),
                          ...
                          children: [
                              ...
                              Expanded(
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Text(p['company'] as String,
                                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Color(0xFF0F172A))),
                                  Text(p['category'] as String, style: const TextStyle(color: Color(0xFF64748B), fontSize: 11)),
                                ]),
                              ),
                              Container(
                                ...
                                child: const Text('Pendente', style: TextStyle(fontSize: 10, color: Color(0xFFD97706), fontWeight: FontWeight.w600)),
                              ),
                            ]),
                            const SizedBox(height: 8),
                            Text('Serviço: ${p['service']}', style: const TextStyle(fontSize: 12, color: Color(0xFF64748B))),
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                ...
                                label: const Text('Avaliar Agora', style: TextStyle(fontWeight: FontWeight.w600)),
                                style: ElevatedButton.styleFrom(
                                  backgroundColor: const Color(0xFF2563EB),
                                  foregroundColor: Colors.white,

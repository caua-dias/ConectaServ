...
import 'app_shell.dart'; // <-- Importação do AppShell
...
class _SearchResultsPageState extends State<SearchResultsPage> {
  ...
  final List<String> _categorias =
      Categoria.values.map((c) => _formatarNomeCategoria(c)).toList();
  ...
  List<Map<String, dynamic>> get _resultadosMapeados {
    ...
    for (var emp in empresas) {
      final servicosEmpresa =
          servicos.where((s) => s.idEmpresa == emp.idEmpresa).toList();
      final categoriasEmpresa = servicosEmpresa
          .map((s) => _formatarNomeCategoria(s.categoria))
          .toSet()
          .toList();
      String categoriaPrincipal =
          categoriasEmpresa.isNotEmpty ? categoriasEmpresa.first : 'Diversos';
      ...
      if (servicosEmpresa.isNotEmpty) {
        ...
        if (minPrice == maxPrice) {
          ...
          priceRange =
              'R\$ ${minPrice.toStringAsFixed(0)} - R\$ ${maxPrice.toStringAsFixed(0)}';
        }
      }
      ...
      mapList.add({
        ...
        'description': emp.descricao ??
            'Empresa prestadora de serviços cadastrada na plataforma.',
        'services': servicosEmpresa.map((s) => s.descricao).take(3).toList(),
    ...
    return mapList.where((emp) {
      ...
      bool matchSearch = q.isEmpty ||
          ...
          (emp['services'] as List<String>)
              .any((s) => s.toLowerCase().contains(q));

      bool matchCat = _selectedCategorias.isEmpty ||
          (emp['allCategories'] as List<String>)
              .any((c) => _selectedCategorias.contains(c));

      bool matchLocal =
          _selectedLocais.isEmpty || _selectedLocais.contains(emp['location']);
        ...
        children: [
          ...
          ...(_locais.map((loc) => CheckboxListTile(
                ...
                onChanged: (v) {
                  setState(() {
                    if (v == true)
                      _selectedLocais.add(loc);
                    else
                      _selectedLocais.remove(loc);
                  });
                },
              ))),
          const SizedBox(height: 16),
  ...
  Widget build(BuildContext context) {
    ...
    // ── Envolvemos o Scaffold com o AppShell ──
    return AppShell(
      child: Scaffold(
              ...
              child: Row(
                children: [
                  ...
                  DropdownButton<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                          value: 'relevancia', child: Text('Mais Relevantes')),
                      DropdownMenuItem(
                          value: 'rating', child: Text('Melhor Avaliadas')),
                      DropdownMenuItem(value: 'nome', child: Text('Nome A-Z')),
                    ],
                    onChanged: (v) =>
                        setState(() => _ordenacao = v ?? 'relevancia'),
                  ),
                ],
              ),
            ),
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Center(
                                            child: Text(emp['emoji'] as String,
                                                style: const TextStyle(
                                                    fontSize: 28))),
                                      ),
                                      const SizedBox(width: 12),
